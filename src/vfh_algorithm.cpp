...
void VFH_Algorithm::Init()
{
    ...
    cells=(unsigned int)(floor(_vfhParams.window_size/_vfhParams.cell_size));
...
NHMotion_t VFH_Algorithm::ComputeMotion(double velocity, double direction, Pose_t &robot, Point_t &goal)
{
    ...
    {
		motion.linear=velocity*cos(direction);
		motion.angular=velocity*sin(direction)/_motionParams.off_center_dist;
    }
    return motion;
}
